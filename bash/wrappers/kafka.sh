...
PID_FILE="/run/kafka.pid"
SERVICE_NAME="Kafka"
SERVICE_ROOT="/opt/kafka"
SERVICE_BIN=${SERVICE_ROOT}/bin

service_pid() {
    echo `ps ax | grep -i "kafka.Kafka" | grep -v grep | awk '{print $1}'`
}
...
service_start() {
    ...
    if [ -n "$pid" ]; then
        ...
        ${SERVICE_BIN}/kafka-server-start.sh -daemon ${SERVICE_ROOT}/config/kafka_cupis.properties
        ${pid} > ${PID_FILE}
